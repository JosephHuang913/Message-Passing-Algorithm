...
#define Num_codeword 409				// number of codewords per packet
...
const long double vc = 120.0;						/* speed of vehicle in km/hr */
...
{
	time_t  ti, start, end;
	int i, j, q, l, m, p, r, x, z, t, d, *data, **data_bit, **Ak, *S_random, min1[4], min2[4];
	...
	long double *gain, ****fade;
	long double ***H_f, *fft, AVE_CH;
...
/*			for(t=0; t<N_fft; t++)
			{
				...
				for(i=0; i<R; i++)
					...
					{
						RCS[t][i][j] = 1;
						AVE_CH += (powl(H_f[i][j][2*t],2.0) + powl(H_f[i][j][2*t+1],2.0));
					}
				...
				for(i=0; i<R; i++)
					for(j=0; j<T; j++)
						if((powl(H_f[i][j][2*t],2.0)+powl(H_f[i][j][2*t+1],2.0)) < AVE_CH/5.0)
							RCS[t][i][j] = 0;
			}
*/
				...
				for(d=0; d<6; d++)
				{
					...
					for(i=0; i<R; i++)
						for(j=0; j<T; j++)
						{
							...
							if(flag == 1)
							{
								if((powl(H_f[i][j][2*t],2.0)+powl(H_f[i][j][2*t+1],2.0)) <= (powl(H_f[min1[d]][min2[d]][2*t],2.0)+powl(H_f[min1[d]][min2[d]][2*t+1],2.0)))
								{
									min1[d] = i;
									min2[d] = j;
								}
