...
/* 2 by 2 QPSK                                                                                           */
/* MIMO Detector: MAP Detector with Sum-Product Rule                            */
...
void MAP_detector(long double ***, long double *, long double, long double *);
...
{
	time_t  ti, start, end;
	int i, j, k, l, p, /*s1, s2,*/ t, d, *data, **data_bit, **Ak, *S_random, *Dk;
	int **err_count, *delay, **coded_bit, *interleaved, *inter, *coded, *bit, *de_inter;
	long double Eb_No, snr, noise_pwr, noise[2], ***Yk, **err_rate, Ps;
	long double **sym_I, **sym_Q;
	long double *gain, *****fade, ***F_l, ***Est_sym_I, ***Est_sym_Q;
	long double **Re_ICI_I, **Re_ICI_Q, ***Re_signal, **Sk, *fft, *Out;
	long double *L_in, *L_a_u, *L_a_d, *L_out, *L_o, *LLR, *intrinsic, *extrinsic;
	...
	LLR = new long double[N_fft*T*Mc];					// Log-likelihood Ratio
	intrinsic = new long double[N_fft*T*Mc];			// Intrinsic information
	extrinsic = new long double[N_fft*T*Mc];			// Extrinsic information
	Dk = new int[N_fft*T*Mc];
...
/*========================================================*/
/*  MAP MIMO Detector & Progressive PIC                                                */
...
/*========================================================*/
			...
			for(s2=0; s2<Iterate2; s2++)
			{
					...
					for(t=0; t<N_fft; t++)
						for(i=0; i<R; i++)
						{
							...
							for(d=1; d<=6; d++)
								for(j=0; j<T; j++)
								{
									Re_ICI_I[i][t] += (Est_sym_I[s2-1][j][(N_fft+t-d)%N_fft] * H_I[i][j][t][d] - Est_sym_Q[s2-1][j][(N_fft+t-d)%N_fft] * H_Q[i][j][t][d]);
									Re_ICI_Q[i][t] += (Est_sym_I[s2-1][j][(N_fft+t-d)%N_fft] * H_Q[i][j][t][d] + Est_sym_Q[s2-1][j][(N_fft+t-d)%N_fft] * H_I[i][j][t][d]);
								}
				...
				s1 = 0;
				if(s2 == 0)
					MAP_detector(Yk, intrinsic, noise_pwr, LLR);
				else
					MAP_detector(Re_signal, intrinsic, noise_pwr, LLR);
				...
				for(i=0; i<N_fft*T*Mc; i++)
				{
					Out[S_random[i]] = L_o[i] + L_a_u[i];
      				//intrinsic[S_random[i]] = L_o[i];		// MAP
					intrinsic[S_random[i]] = 0.0;			// ML
				}
...
void MAP_detector(long double ***Received, long double *intrinsic, long double noise_pwr, long double *LLR)
{
/***************************************************/
/* MAP Equalizer for MIMO channel (QPSK) */
/***************************************************/
	//const float Pi = 3.14159265358979;
	int i, j, t, l, x, z, *data;
	long double **a_priori, p[2], P[2], sum, sum1, sum0, *sym_I, *sym_Q, **Y;

	data = new int[Mc*T];
	a_priori = new long double*[N_fft*T*Mc];			// a-priori probability
	for(i=0; i<N_fft*T*Mc; i++)
   		a_priori[i] = new long double[2];
	sym_I = new long double[T];
	sym_Q = new long double[T];
	Y = new long double*[R];
	for(i=0; i<R; i++)
   		Y[i] = new long double[2];
	...
	for(l=0; l<N_fft; l++)
	{
   		...
   		{
   			sum1 = sum0 = 0.0;
   			for(x=0; x<(int)pow(2.0,Mc*T-1); x++)
			{
   				t= 0;
      			for(j=0; j<Mc*T; j++)
         			if(j!=i)
				...
				P[0] = P[1] = 1.0;
   				for(j=0; j<Mc*T; j++)
      	   			if(j!=i)
         			{
         				P[1] *= a_priori[j+l*Mc*T][data[j]];
            			P[0] *= a_priori[j+l*Mc*T][data[j]];
					}
   				P[1] *= a_priori[i+l*Mc*T][1];
      			P[0] *= a_priori[i+l*Mc*T][0];

				// Channel Observation (Numerator)
   				data[i] = 1;
      			for(j=0; j<T; j++)
         		{
         			sym_I[j] = (2*data[2*j]-1)/sqrt(2.0);
	         		sym_Q[j] = (2*data[2*j+1]-1)/sqrt(2.0);
   				}
      			...
      			for(j=0; j<R; j++)
         		{
	         		...
	         		for(t=0; t<T; t++)
   					{
      	   				Y[j][0] += (sym_I[t] * H_I[j][t][l][0] - sym_Q[t] * H_Q[j][t][l][0]);
         				Y[j][1] += (sym_I[t] * H_Q[j][t][l][0] + sym_Q[t] * H_I[j][t][l][0]);
            		}
				}
      			...
      			for(j=0; j<R; j++)
         			for(z=0; z<2; z++)
         				sum += powl(Received[j][l][z]-Y[j][z],2.0);

				p[1] = exp(-sum/(2.0*noise_pwr))/powl(2.0*Pi*noise_pwr,R);

   				// Channel Observation (Denominator)
      			data[i] = 0;
         		for(j=0; j<T; j++)
				{
   	      			sym_I[j] = (2*data[2*j]-1)/sqrt(2.0);
				...
				p[0] = exp(-sum/(2.0*noise_pwr))/powl(2.0*Pi*noise_pwr,R);

   				sum1 += p[1] * P[1];
      			sum0 += p[0] * P[0];
			}

   	 		LLR[i+l*Mc*T] = log(sum1 / sum0);
		}
	}
