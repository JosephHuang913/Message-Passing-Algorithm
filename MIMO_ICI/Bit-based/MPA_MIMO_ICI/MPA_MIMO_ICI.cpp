...
{
	time_t  ti, start, end;
	int d, i, j, k, l, m, n, p, r, s, t, x, z, *data, ***data_bit, ***Ak, *err_count, ICI, B, X;
	...
	for(i=0; i<R; i++)
		for(j=0; j<T; j++)
			for(l=0; l<path_num; l++)
				fade[i][j][l] = new double*[N_fft+CP_length+delay_spread];
	for(i=0; i<R; i++)
		for(j=0; j<T; j++)
			for(l=0; l<path_num; l++)
	...
	for(snr=5; snr<=15; snr+=5)
	{
		...
		do
		{
			...
			for(i=0; i<T; i++)
				...
				{
					// QPSK Mapping
					sym_I[i][l] = (2*data_bit[i][l][0]-1)/sqrt(2.0);
         			sym_Q[i][l] = (2*data_bit[i][l][1]-1)/sqrt(2.0);	
				}
			...
			for(i=0; i<R; i++)
				for(j=0; j<T; j++)
				{
					for(l=0; l<path_num; l++)
					{
						// FFT of l_th path with respect to time-variations
						for(t=0; t< N_fft; t++)
						{
							fft[2*t+1] = gain[l] * fade[i][j][l][t+CP_length][0];
							fft[2*t+2] = gain[l] * fade[i][j][l][t+CP_length][1];
						}
					...
					for(t=0; t<N_fft; t++)
						if(t <= D)
						{
							...
							for(d=0; d<ICI; d++)
							{
								...
								for(l=0; l<path_num; l++)
								{
									H_I[i][j][t][d] += F_l[l][(N_fft+t-d)%N_fft][0] * cos(2*Pi*delay[l]*(t-((N_fft+t-d)%N_fft))/(float)N_fft) + F_l[l][(N_fft+t-d)%N_fft][1] * sin(2*Pi*delay[l]*(t-((N_fft+t-d)%N_fft))/(float)N_fft);
									H_Q[i][j][t][d] += F_l[l][(N_fft+t-d)%N_fft][1] * cos(2*Pi*delay[l]*(t-((N_fft+t-d)%N_fft))/(float)N_fft) - F_l[l][(N_fft+t-d)%N_fft][0] * sin(2*Pi*delay[l]*(t-((N_fft+t-d)%N_fft))/(float)N_fft);
								}
						...
						else if(t >= N_fft-D-1)
						{
							...
							for(d=0; d<ICI; d++)
							{
								...
								for(l=0; l<path_num; l++)
								{
									H_I[i][j][t][d] += F_l[l][(N_fft+D-d)%N_fft][0] * cos(2*Pi*delay[l]*(t-((N_fft+D-d)%N_fft))/(float)N_fft) + F_l[l][(N_fft+D-d)%N_fft][1] * sin(2*Pi*delay[l]*(t-((N_fft+D-d)%N_fft))/(float)N_fft);
									H_Q[i][j][t][d] += F_l[l][(N_fft+D-d)%N_fft][1] * cos(2*Pi*delay[l]*(t-((N_fft+D-d)%N_fft))/(float)N_fft) - F_l[l][(N_fft+D-d)%N_fft][0] * sin(2*Pi*delay[l]*(t-((N_fft+D-d)%N_fft))/(float)N_fft);
								}
	   				...
	   				for(j=0; j<R; j++)
						for(t=0; t<N_fft; t++)
							for(d=0; d<2*D+1; d++)
      							for(k=0; k<2; k++)
   								{
   									Q_np[i][j][t][d][m][k] = 0.5;
									R_pn[j][i][t][d][m][k] = 0.5;
								}
							...
							for(d=0; d<ICI; d++)
								for(j=0; j<T; j++)
									for(m=0; m<Mc; m++)
									{
											...
											for(x=0; x<X; x++)
   											{
												...
												data[d*Mc*T+j*Mc+m] = k;
         										for(n=0; n<B; n++)
   												...
   												for(r=0; r<T; r++)
													for(l=0; l<ICI; l++)
      												{
         	            								sym_I[r][l] = (2*data[l*Mc*T+r*Mc]-1)/sqrt(2.0);
				            							sym_Q[r][l] = (2*data[l*Mc*T+r*Mc+1]-1)/sqrt(2.0);
													}
												...
												for(l=0; l<ICI; l++)	
													for(r=0; r<T; r++)
													{
									           			Y[0] += (sym_I[r][l] * H_I[i][r][t][l] - sym_Q[r][l] * H_Q[i][r][t][l]);
														Y[1] += (sym_I[r][l] * H_Q[i][r][t][l] + sym_Q[r][l] * H_I[i][r][t][l]);
													}
				                     			...
				                     			for(l=0; l<ICI; l++)
													for(r=0; r<T; r++)
														for(n=0; n<Mc; n++)
									     					if(l*Mc*T+r*Mc+n != d*Mc*T+j*Mc+m)
																P *= Q_np[r][i][l][D-l+t][n][data[l*Mc*T+r*Mc+n]];
										...
										R_pn[i][j][t][d+(D-t)][m][1] = RQ_tmp[1] / (RQ_tmp[0] + RQ_tmp[1]);
										R_pn[i][j][t][d+(D-t)][m][0] = RQ_tmp[0] / (RQ_tmp[0] + RQ_tmp[1]);
									}
						}
						else if(t >= N_fft-D-1)
						{
							...
							for(d=0; d<ICI; d++)
								for(j=0; j<T; j++)
									for(m=0; m<Mc; m++)
									{
												...
												for(l=0; l<ICI; l++)
				                     			...
				                     			for(l=0; l<ICI; l++)
													for(r=0; r<T; r++)
														for(n=0; n<Mc; n++)
									     					if(l*Mc*T+r*Mc+n != d*Mc*T+j*Mc+m)
																P *= Q_np[r][i][l+(t-D)][2*D-l][n][data[l*Mc*T+r*Mc+n]];
										...
										R_pn[i][j][t][d][m][1] = RQ_tmp[1] / (RQ_tmp[0] + RQ_tmp[1]);
										R_pn[i][j][t][d][m][0] = RQ_tmp[0] / (RQ_tmp[0] + RQ_tmp[1]);
									}
						}
						else
						{
										...
										RQ_tmp[0] = RQ_tmp[1] = 1.0;
           								for(l=0; l<ICI; l++)
               								for(n=0; n<R; n++)
           										if(n != i || l != d)
												{
		                 							RQ_tmp[1] *= R_pn[n][j][l][D-l+t][m][1];
													RQ_tmp[0] *= R_pn[n][j][l][D-l+t][m][0];
												}
												
										Q_np[j][i][t][d+(D-t)][m][1] = RQ_tmp[1]/(RQ_tmp[1] + RQ_tmp[0]);
										Q_np[j][i][t][d+(D-t)][m][0] = RQ_tmp[0]/(RQ_tmp[1] + RQ_tmp[0]);
									}
							}
							else if(t >= N_fft-D-1)
							{
										...
										RQ_tmp[0] = RQ_tmp[1] = 1.0;
           								for(l=0; l<ICI; l++)
               								for(n=0; n<R; n++)
           										if(n != i || l != d)
												{
		                 							RQ_tmp[1] *= R_pn[n][j][l+(t-D)][2*D-l][m][1];
													RQ_tmp[0] *= R_pn[n][j][l+(t-D)][2*D-l][m][0];
												}
												
										Q_np[j][i][t][d][m][1] = RQ_tmp[1]/(RQ_tmp[1] + RQ_tmp[0]);
										Q_np[j][i][t][d][m][0] = RQ_tmp[0]/(RQ_tmp[1] + RQ_tmp[0]);
									}
							}
							else
							{
						...
						for(m=0; m<Mc; m++)
						{
							...
							for(i=0; i<R; i++)
								for(l=0; l<ICI; l++)
               						if(t <= D)
									{
	               						RQ_tmp[1] *= R_pn[i][j][l][D-l+t][m][1];
										RQ_tmp[0] *= R_pn[i][j][l][D-l+t][m][0];
									}
									else
									{
										RQ_tmp[1] *= R_pn[i][j][l+(t-D)][2*D-l][m][1];
										RQ_tmp[0] *= R_pn[i][j][l+(t-D)][2*D-l][m][0];
									}
            				...
            				err_count[s] += Error_count(data_bit[j][t][m], Ak[j][t][m]);
						}
			}
	
			p++;
		} while(err_count[Iteration-1] <= 500 && p < num_packet);
