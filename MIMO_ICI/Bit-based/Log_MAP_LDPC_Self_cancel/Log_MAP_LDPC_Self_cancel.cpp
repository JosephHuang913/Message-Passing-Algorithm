...
#define Num_packet 5000					// number of packets simulated
//#define Num_codeword 204			// number of codewords per packet (16-QAM)
#define Num_codeword 102				// number of codewords per packet (QPSK)
...
const int Iterate1 = 10;						// For Message Passing MIMO Detection
...
int main(void)
{
	...
	long double *gain, *****fade, ***F_l, ****H_I, ****H_Q;
	long double ***Sk, *fft, *Out;
	...
	Sk = new long double**[R];
	for(i=0; i<R; i++)
   		Sk[i] = new long double*[N_fft/2];
	for(i=0; i<R; i++)
		for(l=0; l<N_fft/2; l++)
			Sk[i][l] = new long double[2];
...
/*===========================================*/
/* S-random interleaver and de-interleaver (S = 32 or 45) */
/*===========================================*/
	fopen_s(&s_inter, "s_random.log", "r");
	for(i=0; i<(N_fft/2)*T*Mc; i++)		// Interleaver
   		fscanf_s(s_inter, "%d %d", &de_inter[i], &S_random[i]);
	for(i=0; i<(N_fft/2)*T*Mc; i++)		// De-interleaver
...
/*======================*/
	...
	for(snr=14; snr<=16; snr+=2)
	{
		...
		do
		{
			...
			// Zero padding
			for(l=Num_codeword*Cn; l<(N_fft/2)*T*Mc; l++)
				inter[l] = 0;

			// Interleaving: (2048, 32) S-random interleaver
			// Interleaving: (4096, 45) S-random interleaver
			for(i=0; i<(N_fft/2)*T*Mc; i++)
			...
			for(l=0; l<(N_fft/2); l++)
   				for(i=0; i<T; i++)
	   			{
					...
					sym_I[i][2*l] = (2*interleaved[l*Mc*T+i*Mc]-1)/sqrt(2.0);
         			sym_Q[i][2*l] = (2*interleaved[l*Mc*T+i*Mc+1]-1)/sqrt(2.0);
					sym_I[i][2*l+1] = -sym_I[i][2*l];
         			sym_Q[i][2*l+1] = -sym_Q[i][2*l];
/*
					// 16-QAM Mapping	
...
/*========================================================*/
			...
			for(i=0; i<R; i++)
				for(t=0; t<N_fft/2; t++)
				{
					Sk[i][t][0] = Yk[i][2*t][0] - Yk[i][2*t+1][0];
					Sk[i][t][1] = Yk[i][2*t][1] - Yk[i][2*t+1][1];
				}
			    	...
			    	for(i=0; i<R; i++)
						for(j=0; j<T; j++)
							for(m=0; m<Mc; m++)
       						{
								flag = 0;
								for(r=Num_codeword*Cn; r<(N_fft/2)*Mc*T; r++)
		           						...
		           						for(x=0; x<powl(2.0,Mc*T-1.0); x++)
	   									{
			  								...
			  								for(r=0; r<Mc*T; r++)
				   		         				if(r != j*Mc+m)
       											{
							  			  			data[r] = (x>>l) & 1;
							 						l++;
													for(d=Num_codeword*Cn; d<(N_fft/2)*Mc*T; d++)
...
/*
											...
											for(r=0; r<T; r++)
											{
							   					Y[0] += (I[r] * H_I[i][r][2*t][0] - Q[r] * H_Q[i][r][2*t][0]);
												Y[1] += (I[r] * H_Q[i][r][2*t][0] + Q[r] * H_I[i][r][2*t][0]);
												Y[0] += (I[r] * H_I[i][r][2*t+1][0] - Q[r] * H_Q[i][r][2*t+1][0]);
												Y[1] += (I[r] * H_Q[i][r][2*t+1][0] + Q[r] * H_I[i][r][2*t+1][0]);
											}
				...
				for(t=0; t<(N_fft/2); t++)
					for(j=0; j<T; j++)
						...
						{
							flag = 0;
							for(i=Num_codeword*Cn; i<(N_fft/2)*Mc*T; i++)
			...
			p++;
		} while(err_count[Iterate1-1] <= 10000 && p < Num_packet);

		// Statistics and records
		cout << "Error Rate = " << endl;
		for(s1=0; s1<Iterate1; s1++)
		{
   			err_rate[s1] = err_count[s1] / (long double)(Cn*Num_codeword*p);
      		printf("%e\n", err_rate[s1]);
		}
