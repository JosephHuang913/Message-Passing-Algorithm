...
/* ICI Canceller: Progressive PIC (PPIC), turn off                                       */
/*========================================================*/
					...
					for(t=0; t<N_fft; t++)
						for(i=0; i<R; i++)
						{
							...
							*/
							...
							Re_signal[i][t][0] = Yk[i][t][0];
							Re_signal[i][t][1] = Yk[i][t][1];
						}
				}
				...
				for(s1=0; s1<Iterate1; s1++)
					for(t=0; t<N_fft; t++)
					{
		           						...
		           						for(x=0; x<pow(2.0,Mc*T-1.0); x++)
	   									{
											...
											for(z=0; z<2; z++)
												if(s2 == 0)
													...
													sum += pow(Re_signal[i][t][z]-Y[z],2.0);
						...
						for(j=0; j<Mc*T; j++)
						{
	            			...
	            			err_count[s2][s1] += Error_count(data_bit[t][j], Ak[j]);
						}
					}
				/*
				for(t=0; t<N_fft; t++)
