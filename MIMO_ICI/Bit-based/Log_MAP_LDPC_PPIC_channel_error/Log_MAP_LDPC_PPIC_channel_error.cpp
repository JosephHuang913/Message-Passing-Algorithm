...
{
	time_t  ti, start, end;
	int i, j, k, q, l, m, p, r, /*s1, s2,*/ s3, x, z, t, d, *data, **data_bit, **Ak, *S_random;
	int **err_count, *delay, **coded_bit, flag, *interleaved, *inter, *coded, *bit, *de_inter;
	long double Eb_No, snr, noise_pwr, noise[2], Y[2], ***Yk, **err_rate, Ps, sigma_e;
	...
	long double ****H_est_I, ****H_est_Q, ****ICI_gain;
	FILE *ber, *records, *s_inter, *parity_chk;

	start = time(NULL);
	printf("BER Performance of Bit-based Message Passinlg in MIMO Channel with Channel Estimation Error\n");
	...
	fprintf(records, "BER Performance of Bit-based Message Passinlg in MIMO Channel with Channel Estimation Error\n");
					...
					for(t=0; t<N_fft; t++)
						for(i=0; i<R; i++)
						{
							...
							for(d=1; d<=s2; d++)
							{
								for(j=0; j<T; j++)
								{
									Re_ICI_I[i][t] += (Est_sym_I[s3][j][(N_fft+t-d)%N_fft] * H_est_I[i][j][t][d] - Est_sym_Q[s3][j][(N_fft+t-d)%N_fft] * H_est_Q[i][j][t][d]);
									Re_ICI_Q[i][t] += (Est_sym_I[s3][j][(N_fft+t-d)%N_fft] * H_est_Q[i][j][t][d] + Est_sym_Q[s3][j][(N_fft+t-d)%N_fft] * H_est_I[i][j][t][d]);
								}
								s3--;
							}
